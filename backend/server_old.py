...
# ROOT_DIR = Path(__file__).parent.parent  # Go up to workspace root
...
async def get_ai_stock_analysis(stock_symbol: str, stock_name: str, sector: str, analysis_type: str = "hybrid") -> Dict[str, Any]:
    """Use Google Gemini to analyze a stock"""
...
async def generate_trade_recommendation(stock_symbol: str, stock_name: str, sector: str) -> Optional[Dict[str, Any]]:
    """Generate a trade recommendation using Google Gemini AI"""
    import google.generativeai as genai
    import json
    import re
    ...
    prompt = f"""
    You are a trading algorithm that generates specific BUY or SELL recommendations.
...
async def startup_event():
    """Initialize stocks on app startup"""
    try:
        # Check if stocks collection is empty
        stock_count = await db.stocks.count_documents({})
        if stock_count == 0:
            logger.info("Stocks collection is empty. Initializing with predefined stocks...")
            stocks = []
            ...
            if stocks:
                ...
                logger.info(f"✓ Initialized {len(stocks)} stocks successfully")
        else:
            logger.info(f"✓ Found {stock_count} stocks in database")
    except Exception as e:
        logger.error(f"✗ Failed to initialize stocks on startup: {e}")
